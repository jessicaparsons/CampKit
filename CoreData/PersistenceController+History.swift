...
extension PersistenceController {
    func listTransactions(from notification: Notification) -> [NSPersistentHistoryTransaction] {
        ...
        if let transactions = notification.userInfo?[UserInfoKey.transactions] as? [NSPersistentHistoryTransaction] {
            let listEntityName = PackingList.entity().name
            for transaction in transactions where transaction.changes != nil {
                for change in transaction.changes! where change.changedObjectID.entity.name == listEntityName {
